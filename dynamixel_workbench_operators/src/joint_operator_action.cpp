...
#define max_number_of_joints 40 //change this value for more number of joints
...
JointOperator::JointOperator()
  ...
  is_loop_(false)
{
	 std::string yaml_file="/home/robin/catkin_ws/src/dynamixel-workbench/dynamixel_workbench_operators/config/all_dynamixels.yaml";
...
void ExampleActionServer::executeCB(const actionlib::SimpleActionServer<dynamixel_workbench_operators::joint_opAction>::GoalConstPtr& goal) {
	...
	else if (0==goal->input.compare("six"))
	{
			ROS_INFO("inside service call back with pose five");
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+5));  
			result_message.output = "sixth pose"; // we'll use the member variable result_, defined in our class
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("seven"))
	{
			ROS_INFO("inside service call back with pose five");
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+6));  
			result_message.output = "seventh pose"; // we'll use the member variable result_, defined in our class
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("rock"))
	{
			ROS_INFO("inside service call back with pose rock");
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+7));  
			result_message.output = "rock pose"; // we'll use the member variable result_, defined in our class
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("hello"))
	{
			ROS_INFO("inside service call back with pose hello");
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+8));  
			result_message.output = "hello pose"; // we'll use the member variable result_, defined in our class
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("ten"))
	{
			ROS_INFO("inside service call back with pose five");
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+9));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("left_hand_intro_final"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+10));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("left_hand_home"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+11));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("right_hand_intro_final"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+12));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("right_hand_home"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+13));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("head_left_intro"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+14));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("head_right_intro"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+15));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("head_home"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+16));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("attention"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+17));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("thinking"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+18));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("weather"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+19));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("thankyou"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+20));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("group_greeting"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+21));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("fold_both_elbow"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+22));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("move_forward"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+23));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("rotate_both_outward"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+24));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("lift_both_outwards"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+25));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("move_neck"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+26));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("talking_hand_1_0"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+27));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("talking_hand_1_1"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+28));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("talking_hand_1_2"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+29));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("talking_hand_1_3"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+30));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("talking_hand_1_4"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+31));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("talking_hand_1_5"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+32));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("talking_hand_1_6"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+33));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("talking_hand_1_7"))
	{
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+34));  
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
