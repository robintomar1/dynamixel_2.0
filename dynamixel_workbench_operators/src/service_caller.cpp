...
{ 
	...
	else if (strcmp((msg->data.c_str()),"four")==0)
	    {
	    	ROS_INFO("inside topic callback inside four");
	    	request_object.pose_name="four";
	    	/*call the service with argument : "three"*/
	    	bool success = pose_Client.call( request_object, response_object ) ;
	    	// Check for success and use the response .
		if ( success ) 
	...
	else if (strcmp((msg->data.c_str()),"five")==0)
	    {
	    	ROS_INFO("inside topic callback inside five");
	    	request_object.pose_name="five";
	    	/*call the service with argument : "three"*/
	    	bool success = pose_Client.call( request_object, response_object ) ;
	    	// Check for success and use the response .
		if ( success ) 
